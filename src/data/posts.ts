...
**1. 人員高流動率**：45% 業務員未滿 1 年年資；逾 80% 業務員年資未滿 2 年，業務經驗難以複製並傳承。系統提供 24/7 對練，預計可減少基礎培訓 2 天/人‧月。

**2. 商品知識需加強**：面對客戶回應時，應對方式可更具彈性與多元性。系統提供即時查詢產品與話術資訊，預計提升新人月平均成交比例約 20%。

**3. 培訓成效不易掌握**：無量化與追蹤系統，難以評估成效與後續優化。系統存取所有練習記錄，收集成功經驗形成標準化建議。

## 三代理協作架構

不是「又一個 chatbot」，而是一個「擬人教練團」：

- **Customer Generation Agent**：負責設定場景，生成客戶 Persona
- **Customer Agent**：在對話中扮演客戶，展現真實的行為與情緒，必要時可掛電話
- **Coach Agent**：在對話結束後，提供評分、評語，甚至示範更好的說法

## 靈魂所在：Persona World

AI 的真實感源於深度心理學模型。我們不僅定義客戶的人口統計資料，更注入他們的心理特質、價值觀與行為模式——表裏層人格模擬人性。

系統抽取 **15 項**社會特徵維度，可生成 **5 億**種客戶樣態。

## 不是給分數，而是給「下一步」

使用 Rubrics 評分方式，提供具體可執行的回饋。它會指出問題，並直接示範「更好的作法」。5 種銷售理論（AIDA / SPIN / FAB / CRM / 28000天）＋ 6 道管理員自訂關卡。

## Context Engineering 是智慧核心

透過四層堆疊的 Context 設計，讓所有 Agent 都能精準理解對話的全局、情境、人物與當下狀態。

Fun fact：Context Engineering 這個詞這一兩個月開始流行，但這項產品在半年前就已經採用 context 交互策略。

## 不只懂對話，更懂「規則」：Ontology + GraphRAG

受到 Palantir 啟發的核心大腦——企業級的「本體論」架構（Ontology）。

**Ontology 的三層架構：**
- **世界是什麼？** 定義業務的名詞（物件、屬性、關係），建立共享知識網路
- **世界如何變化？** 定義業務的動詞（操作、流程、行為），實現從數據到行動的閉環
- **世界如何決策？** 定義決策機制（AI 模型、規則），實現智慧優化與自動化

透過 GraphRAG，將產品資訊、SOP 及所有資訊掛載到節點上，當 Agent 搜尋時能找到相關節點並沿著圖的結構窮舉所有上下游節點及關聯資訊。

## 前瞻：Graph-R1（Agentic GraphRAG + RL）

Graph-R1 提出一個 Agentic GraphRAG 框架，賦予 LLM 與知識庫進行動態互動並從中學習的能力：

**核心革新一：輕量級 Hypergraph** — 比起傳統圖，一條邊可連結更多實體，有效加速搜尋速度和降低建置成本。

**核心革新二：多輪對話式探索** — 讓 Agent 能更像人一樣「追問」與「挖掘」。

**核心革新三：End-to-End 強化學習** — 採用端到端的獎勵機制，透過 RL 對從檢索到生成的整個 AI Agent 流程進行優化，Agent 每一次成功的探索都能獲得獎勵，持續找到更優的搜尋策略。

## 數據驅動，洞見未來

當所有互動都發生在同一個平台上，就能獲得前所未有的管理洞察力：

- **Training Analytics**：跨市場比較業務學員的上手時間與各階段銷售的平均分數
- **Sales Quality**：識別最常見的客戶異議、高風險對話模式
- **Market Intelligence**：分析不同市場客戶 Persona 的共同與差異

## 索取完整投影片

對這個主題感興趣？歡迎來信索取完整投影片。
