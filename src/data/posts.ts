...
工具在變，角色就有機會跟著變，接下來幾年，誰能把「工具進化」跟「組織能力進化」綁在一起，誰就會在這一波 AI 浪潮裡走得比較穩`
    },
    {
        ...
        excerpt: "週日第一次參加 DevFest Taipei 2025，分享 AI 教練——擬人教練團的企業落地，獲得高品質的意見交流。",
        date: "2025-12-02",
        author: "Nils Liu",
        coverImage: "/images/blog/devfest-taipei-2025.jpg",
        content: `週日第一次參加DevFest Taipei 2025，分享我的題目後獲得了高品質的意見交流，很高興有這個機會參與這場盛會，接下來持續完善整個平台，期待下次DevFest與大家分享更豐富的作品
...
這不是聊天機器人，而是一個「擬人教練團（Agents Team）」
用多個 Agent 分工，把海外業務訓練做成 24/7 的個人化對練系統，主打「真實感」的銷售對話模擬（保險＋消費金融）。

目前已有初步成效數字：
- 118 人參與測試
- 滿意度有兩種口徑：問卷 86%（以使用人數為分母）、系統統計 96%（以使用次數為分母）

### 核心互動：三代理協作
- Customer Generation Agent：設定場景、生成客戶 persona
- Customer Agent：扮演客戶（帶情緒與行為，必要時可掛電話）
- Coach Agent：結束後評分與回饋，並示範更好的說法

### 真實感的來源：Persona World（心理層）
不是只靠人口統計資料，而是把心理特質、價值觀、行為模式一起注入，讓客戶更像真人。

### 技術觀點：Context Engineering 是智慧核心
用「多層 Context 堆疊」讓所有 Agent 在同一世界觀下理解情境、人物、狀態，提升一致性與表現。

### 更大的願景：從 AI 教練擴成企業級 AI Agent 平台
多代理、Persona、Rubrics 都被定位成可複用核心能力，要平台化成「企業共用能力」。

### "懂規則"的方法：Ontology + GraphRAG
把產品、風險、法規、客戶等「關係」建成圖；GraphRAG 讓 Agent 檢索時能沿圖找上下游關聯並連到文件與 SOP。

### 前瞻方向：Graph-R1（Agentic GraphRAG + RL）
- hypergraph（更快、更省成本）
- 多輪對話式探索（更會問、更會挖）
- 端到端強化學習（优化「找→讀→答」的整條路徑）

### 一頁式 Takeaways
- 定位：AI 教練 = 企業落地的 Agents Team 訓練平台
- 證據：118 人測試；滿意度 86%（問卷）/ 96%（系統）
- 方法：三代理 + Persona World + Rubrics + Context Engineering
- 能力：Ontology + GraphRAG 讓系統「懂規則、懂上下文」
- 路線：Graph-R1（Agentic GraphRAG + RL）→ 數據飛輪
- 價值：訓練可規模化、可量化；並產出管理洞察`
    },
    {
